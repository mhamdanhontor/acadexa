...
def process_pending_notifications() -> int:
    ...
    if settings.WHATSAPP_PROVIDER.lower() in ("manual", "admin_web"):
        # Manual / Admin Web mode: jobs remain PENDING until the admin dispatches them from WhatsApp Web / Desktop
        return 0
    ...
    try:
        ...
        for job in jobs:
            ...
            # Throttle dispatches with configured delay (e.g. 10s) to avoid spam/rate-limits
            if (
                settings.WHATSAPP_DISPATCH_DELAY_SECONDS > 0
                and settings.WHATSAPP_PROVIDER.lower() not in ("fake", "manual", "admin_web")
                and processed < len(jobs)
            ):
                import time
                time.sleep(settings.WHATSAPP_DISPATCH_DELAY_SECONDS)
